...
function otsuThresholding(inputImage, width, height) {
    let grayImage = new cv.Mat();
...
function start_benchmarks() {
    ...
    timings = timeToMsAndFPS(laplacian(inputImage, 1920, 1080));
    document.getElementById("Laplacian").innerHTML = "Laplacian 1920x1080: "+ timings['ms'] + "ms, " + timings['fps'] + " fps";

    timings = timeToMsAndFPS(otsuThresholding(inputImage, 1920, 1080));
    document.getElementById("OTSUThresh").innerHTML = "OTSU thresholding 1920x1080: "+ timings['ms'] + "ms, " + timings['fps'] + " fps";
